...
module Lib3(parseDocument, GameStart(..), Hint(..), gameStart, hint) where

import Types ( Document(..), FromDocument, fromDocument, Coord (..) )
import Lib1 (State(..), GameRow (GameDataRow, ColumnInfoRow), dynamicMap, dmapGet, deList, replaceRow, CellType(..))
import Lib2 (renderTabs, stripFrontWhitespace)
...
parseDocument str = do
    starter <- (parseStringExact "---\n" str) `Lib3.or` (parseStringExact "" str)
    parsed <- (parseComplexDoc (snd starter)) `Lib3.or` (parsePrimitiveDoc (snd starter))
    return (fst parsed)
...
validateHint (DMap [(strHead,DMap [(strCol,c),(strRow,r)]),(strTail,doc)]) =
    if strHead /= "head"
        then Left $ "Hint: expected \"head\", but got: \"" ++ strHead ++ "\""
    else if strCol /= "col"
            then Left $ "Hint: expected \"col\", but got: \"" ++ strCol ++ "\""
        else if strRow /= "row"
                then Left $ "Hint: expected \"row\", but got: \"" ++ strRow ++ "\""
            else if strTail /= "tail"
                    then Left $ "Hint: expected \"tail\", but got: \"" ++ strTail ++ "\""
                else validateDInteger c h 9 >>
...
validateList (x:xs) h = do
    ...
    if xs == [] then Right y else validateList xs h
...
parseQuotedString str = do
    quote <- parseChar '\'' str `Lib3.or` parseChar '"' str `Lib3.or` parseChar '`' str
...
parseEmptyString str = parseStringExact "\"\"" str `Lib3.or` parseStringExact "\'\'" str
...
parseNull str = (parseStringExact "null" str `Lib3.or` parseStringExact "~" str) >>= (\(i,r) -> if null r then return (i,r) else parseNL r >> return (i,r))
...
parsePrimitiveDoc str =
    ((DNull,).snd <$> parseNull str) `Lib3.or`
    (first DInteger <$> parseInteger str) `Lib3.or`
    (first DInteger <$> parseNegativeInteger str) `Lib3.or`
    ((DList [],).snd <$> parseEmptyList str) `Lib3.or`
    ((DMap [],).snd <$> parseEmptyMap str) `Lib3.or`
    (bimap DString ('\n' :) <$> parseQuotedString str) `Lib3.or`
    -- ((DString "",).snd <$> parseEmptyString str) `Lib3.or`
    (bimap DString ('\n' :) <$> parseStringWithDelimNoEscape '\n' str) `Lib3.or`
    parseAllString str

parseComplexDoc :: String -> Either String (Document, String)
parseComplexDoc str = parseList 0 str `Lib3.or` parseMap 0 str


parseMap :: Int -> String -> Either String (Document, String)
parseMap level str = do
    tabs <- parseTab level str
    element <- parseMapItemList level (snd tabs) `Lib3.or`
               parseMapItemMap level (snd tabs) `Lib3.or`
...
getMapKey line =
            (parseQuotedString (fst line) >>= (\(i,r) -> (i,).snd <$> parseChar ':' r)) `Lib3.or`
            (parseEmptyString (fst line) >>= (\(i,r) -> (i,).snd <$> parseChar ':' r)) `Lib3.or`
            case parseQuotedString (fst line) of
                Left _ ->  parseStringWithDelimSeq ": " (fst line) `Lib3.or`
...
parseMapItemList level str = do
    ...
    key <- getMapKey line
    _ <- parseNL $ snd key
    list <- parseList (level+1) (snd line) `Lib3.or`
...
parseList level str = do
    tabs <- parseTab level str
    element <- parseListItemList level (snd tabs) `Lib3.or`
               parseListItemMap level (snd tabs) `Lib3.or`
