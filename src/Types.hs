...
arbitraryDString = do
    s <- getSize
    n <- choose (0, min 16 s)
    DString <$> vectorOf n (oneof [arbitraryUpper, arbitraryLower, arbitraryDigit])
...
arbitraryDMap = do
    s <- getSize
    n <- choose (0, min 4 s)
    DMap <$> removeDuplicates <$> (vectorOf n ((,) <$> arbitraryK <*> arbitraryDocument))
    where
        arbitraryK = do
            s <- getSize
            n <- choose (1, min 10 s)
...
removeDuplicates :: [(String, Document)] -> [(String, Document)]
removeDuplicates ((k,v):xs) = case findByKey k xs of
    Nothing -> (k,v) : (removeDuplicates xs)
    Just _ -> removeDuplicates xs 
removeDuplicates [] = [] 

findByKey :: String -> [(String,Document)] -> Maybe Document
findByKey key ((k,v):xs) = if key == k then Just v else (findByKey key xs)
findByKey _ [] = Nothing 
